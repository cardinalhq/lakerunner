...
func ProcessAndSplit(ll *slog.Logger, fh *filecrunch.FileHandle, tmpdir string, ingestDateint int32) (map[SplitKey]HourlyResult, error) {
	type gs struct {
	...
	for {
		...
		if !exists {
			w, err := buffet.NewWriter(fh.File.Name(), tmpdir, fh.Nodes, 0, 0)
			if err != nil {
				return nil, err
			}
			st = &gs{
