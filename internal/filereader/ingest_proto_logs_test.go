...
func TestIngestProtoLogsReader_EmptySlice(t *testing.T) {
	...
	opts := ReaderOptions{
		SignalType:        SignalTypeLogs,
		BatchSize:         1,
		ExemplarProcessor: exemplars.NewProcessor(exemplars.Config{}),
	}
...
func TestIngestProtoLogsReader_SyntheticData(t *testing.T) {
	...
	for i, row := range allRows {
		t.Run(fmt.Sprintf("log_%d", i), func(t *testing.T) {
			...
			assert.Equal(t, "test-log-service", row[wkk.NewRowKey("resource_service_name")])
			_, hasResourceDeploymentEnvironment := row[wkk.NewRowKey("resource_deployment_environment")]
			assert.True(t, hasResourceDeploymentEnvironment, "Should have resource environment")
			assert.Equal(t, "test", row[wkk.NewRowKey("resource_deployment_environment")])

			// Check for scope attributes
			_, hasScopeLoggerType := row[wkk.NewRowKey("scope_logger_type")]
...
func TestIngestProtoLogsReader_SyntheticStructuredData(t *testing.T) {
	...
	for i, row := range allRows {
		t.Run(fmt.Sprintf("structured_log_%d", i), func(t *testing.T) {
			...
			assert.Equal(t, "3.1.4", row[wkk.NewRowKey("resource_service_version")])
			assert.Equal(t, "integration", row[wkk.NewRowKey("resource_deployment_environment")])
			assert.Equal(t, "test-host-01", row[wkk.NewRowKey("resource_host_name")])

			// Check scope attributes from structured data
			assert.Equal(t, "slog", row[wkk.NewRowKey("scope_logger_framework")])
			assert.Equal(t, "json", row[wkk.NewRowKey("scope_logger_output")])
...
func TestIngestProtoLogsReader_MultiResourceSyntheticData(t *testing.T) {
	...
	dbLogs := 0
	for _, row := range allRows {
		serviceName := row[wkk.NewRowKey("resource_service_name")].(string)
		switch serviceName {
		case "web-frontend":
			webLogs++
		case "postgres-db":
