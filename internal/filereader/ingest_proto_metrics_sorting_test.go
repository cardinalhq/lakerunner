...
// TestRowKeyInterning verifies that the RowKey interning works correctly.
func TestRowKeyInterning(t *testing.T) {
	// Verify that the same string produces the same interned key
	key1 := wkk.NewRowKey("metric_name")
	key2 := wkk.NewRowKey("metric_name")
	require.Equal(t, key1, key2, "Same string should produce equal RowKeys")

	// Verify that RowKeyCName equals a freshly created metric_name key
	key3 := wkk.RowKeyCName
	require.Equal(t, key1, key3, "wkk.RowKeyCName should equal wkk.NewRowKey(\"metric_name\")")

	// Test that a row with wkk.RowKeyCName works with ComputeTID
	...
	t.Logf("TID for minimal row: %d", tid)

	// Verify keys are found
	debugKeys := fingerprinter.DebugTIDKeys(row)
	t.Logf("Debug keys: %v", debugKeys)
	require.Len(t, debugKeys, 2, "Should have 2 keys (metric_name and chq_metric_type)")

	t.Logf("RowKey interning works correctly")
}

// TestSortingIngestProtoMetricsReader_TIDConsistency verifies that the TID computed
// by the reader (stored in chq_tid) matches what ComputeTID() returns when called
// on the same row. This is critical because the reader uses ComputeTIDFromOTEL()
// during sorting but ComputeTID() during row building.
func TestSortingIngestProtoMetricsReader_TIDConsistency(t *testing.T) {
	...
	require.NoError(t, err, "Failed to decompress test data")
	require.NoError(t, gzReader.Close())

	// First, parse the OTEL data directly to see what ComputeTIDFromOTEL produces
	metrics, err := parseProtoToOtelMetrics(bytes.NewReader(decompressed))
	require.NoError(t, err, "Failed to parse proto")

	// Get the first datapoint's OTEL keys for debugging
	if metrics.ResourceMetrics().Len() > 0 {
		...
		if rm.ScopeMetrics().Len() > 0 {
			...
			if sm.Metrics().Len() > 0 {
				...
				dpAttrs := fingerprinter.GetDatapointAttributes(metric, 0)
				otelKeys := fingerprinter.DebugTIDKeysFromOTEL(rm.Resource().Attributes(), dpAttrs, metricName, metric.Type())
				t.Logf("Keys from ComputeTIDFromOTEL (%d keys):", len(otelKeys))
				for _, kv := range otelKeys {
					t.Logf("  %s", kv)
				}

				// Also compute the actual TID
				otelTID := fingerprinter.ComputeTIDFromOTEL(rm.Resource().Attributes(), dpAttrs, metricName, metric.Type())
				t.Logf("ComputeTIDFromOTEL result: %d", otelTID)
			}
		}
	}

	// Create the sorting reader
	...
	require.NoError(t, err, "Failed to create sorting reader")
	t.Cleanup(func() { require.NoError(t, reader.Close()) })

	// Read all rows and verify TID consistency
	ctx := context.Background()
	var totalRows int
	var tidMismatches int
	...
	for {
		...
		for i := 0; i < batch.Len(); i++ {
			...
			// Get the TID that the reader computed
			readerTID, ok := row[wkk.RowKeyCTID].(int64)
			require.True(t, ok, "Row %d: chq_tid must be int64, got %T", totalRows, row[wkk.RowKeyCTID])

			// Compute TID from the row using ComputeTID
			computedTID := fingerprinter.ComputeTID(row)

			// They must match!
			if readerTID != computedTID {
				tidMismatches++
				if tidMismatches == 1 {
					// Dump first mismatch row for debugging
					t.Errorf("Row %d: TID mismatch! Reader TID: %d, ComputeTID: %d\nMetric: %v\nType: %v",
						totalRows, readerTID, computedTID,
						row[wkk.RowKeyCName], row[wkk.RowKeyCMetricType])
					// Show exactly what keys ComputeTID would use
					tidKeys := fingerprinter.DebugTIDKeys(row)
					t.Logf("Keys used for ComputeTID (%d keys):", len(tidKeys))
					for _, kv := range tidKeys {
						t.Logf("  %s", kv)
					}
				}
			}
	...
	assert.Equal(t, 0, tidMismatches, "All TIDs should match between reader and ComputeTID()")

	t.Logf("Verified TID consistency for %d rows", totalRows)
}
