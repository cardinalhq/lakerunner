...
// MetricCompactionProcessorV2 implements compaction processing for metrics using the base framework
type MetricCompactionProcessorV2 struct {
	*CommonProcessorBase[*messages.MetricCompactionMessage, messages.CompactionKey]
	store MetricCompactionStore
}

// NewMetricCompactionProcessorV2 creates a new metric compaction processor
func NewMetricCompactionProcessorV2(
	store MetricCompactionStore,
	storageProvider storageprofile.StorageProfileProvider,
	cmgr cloudstorage.ClientProvider,
	cfg *config.Config,
) *MetricCompactionProcessorV2 {
	return &MetricCompactionProcessorV2{
...
func (p *MetricCompactionProcessorV2) Process(ctx context.Context, group *accumulationGroup[messages.CompactionKey], kafkaCommitData *KafkaCommitData) error {
...
func (p *MetricCompactionProcessorV2) ProcessWork(
	ctx context.Context,
	tmpDir string,
...
func (p *MetricCompactionProcessorV2) GetTargetRecordCount(ctx context.Context, groupingKey messages.CompactionKey) int64 {
...
func (p *MetricCompactionProcessorV2) performCompaction(ctx context.Context, tmpDir string, storageClient cloudstorage.Client, compactionKey messages.CompactionKey, storageProfile storageprofile.StorageProfile, activeSegments []lrdb.MetricSeg, recordCountEstimate int64) ([]lrdb.MetricSeg, []parquetwriter.Result, error) {
...
func (p *MetricCompactionProcessorV2) uploadAndCreateSegments(ctx context.Context, client cloudstorage.Client, profile storageprofile.StorageProfile, results []parquetwriter.Result, key messages.CompactionKey, inputSegments []lrdb.MetricSeg) ([]lrdb.MetricSeg, error) {
	ll := logctx.FromContext(ctx)
...
func (p *MetricCompactionProcessorV2) atomicDatabaseUpdate(ctx context.Context, oldSegments, newSegments []lrdb.MetricSeg, kafkaCommitData *KafkaCommitData, key messages.CompactionKey) error {
	ll := logctx.FromContext(ctx)
...
func (p *MetricCompactionProcessorV2) getHourFromTimestamp(timestampMs int64) int16 {
	return int16((timestampMs / (1000 * 60 * 60)) % 24)
}

func (p *MetricCompactionProcessorV2) markSegmentsAsCompacted(ctx context.Context, segments []lrdb.MetricSeg, key messages.CompactionKey) error {
	if len(segments) == 0 {
		return nil
	}
...
func (p *MetricCompactionProcessorV2) logCompactionOperation(ctx context.Context, storageProfile storageprofile.StorageProfile, inputSegments, outputSegments []lrdb.MetricSeg, results []parquetwriter.Result, key messages.CompactionKey, recordEstimate int64) error {
