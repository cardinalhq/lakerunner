...
func newTraceIngestBoxerProcessor(cfg *config.Config, store BoxerStore) *TraceIngestBoxerProcessor {
...
func (b *TraceIngestBoxerProcessor) Process(ctx context.Context, group *accumulationGroup[messages.IngestKey], kafkaOffsets []lrdb.KafkaOffsetInfo) error {
	...
	workID, err := workqueue.AddBundle(ctx, b.store, config.BoxerTaskIngestTraces, group.Key.OrganizationID, group.Key.InstanceNum, bundleBytes)
	if err != nil {
	...
	ll.Info("Successfully enqueued trace ingestion bundle to work queue",
