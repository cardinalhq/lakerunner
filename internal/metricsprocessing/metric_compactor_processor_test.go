...
func TestCreateAggregatingReader_Seglog990(t *testing.T) {
	...
	t.Run("MultipleReaders", func(t *testing.T) {
		...
		if len(readerStack.Readers) > 1 {
			...
			// Create mergesort reader from multiple readers first
			sortKeyProvider := &filereader.MetricSortKeyProvider{}
			mergeReader, err := filereader.NewMergesortReader(ctx, testReaders, sortKeyProvider, 1000)
			require.NoError(t, err, "Failed to create merge reader")
			defer mergeReader.Close()

			aggReader, err := filereader.NewAggregatingMetricsReader(mergeReader, int64(metadata.FrequencyMS), 1000)
