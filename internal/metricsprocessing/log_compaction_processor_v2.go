...
// LogCompactionProcessorV2 implements compaction processing for logs using the base framework
type LogCompactionProcessorV2 struct {
	*CommonProcessorBase[*messages.LogCompactionMessage, messages.LogCompactionKey]
	store LogCompactionStore
}

// NewLogCompactionProcessorV2 creates a new log compaction processor
func NewLogCompactionProcessorV2(
	store LogCompactionStore,
	storageProvider storageprofile.StorageProfileProvider,
	cmgr cloudstorage.ClientProvider,
	cfg *config.Config,
) *LogCompactionProcessorV2 {
	return &LogCompactionProcessorV2{
...
func (p *LogCompactionProcessorV2) Process(ctx context.Context, group *accumulationGroup[messages.LogCompactionKey], kafkaCommitData *KafkaCommitData) error {
...
func (p *LogCompactionProcessorV2) ProcessWork(
	ctx context.Context,
	tmpDir string,
...
func (p *LogCompactionProcessorV2) GetTargetRecordCount(ctx context.Context, groupingKey messages.LogCompactionKey) int64 {
	return p.store.GetLogEstimate(ctx, groupingKey.OrganizationID)
}

// Helper methods from original processor
func (p *LogCompactionProcessorV2) performLogCompactionCore(ctx context.Context, tmpDir string, storageClient cloudstorage.Client, compactionKey messages.LogCompactionKey, storageProfile storageprofile.StorageProfile, activeSegments []lrdb.LogSeg, recordCountEstimate int64) ([]lrdb.LogSeg, []parquetwriter.Result, error) {
...
func (p *LogCompactionProcessorV2) uploadAndCreateLogSegments(ctx context.Context, client cloudstorage.Client, profile storageprofile.StorageProfile, results []parquetwriter.Result, key messages.LogCompactionKey, inputSegments []lrdb.LogSeg) ([]lrdb.LogSeg, error) {
	ll := logctx.FromContext(ctx)
...
func (p *LogCompactionProcessorV2) atomicLogDatabaseUpdate(ctx context.Context, oldSegments, newSegments []lrdb.LogSeg, kafkaCommitData *KafkaCommitData, key messages.LogCompactionKey) error {
	ll := logctx.FromContext(ctx)
...
func (p *LogCompactionProcessorV2) getHourFromTimestamp(timestampMs int64) int16 {
	return int16((timestampMs / (1000 * 60 * 60)) % 24)
}

func (p *LogCompactionProcessorV2) markLogSegmentsAsCompacted(ctx context.Context, segments []lrdb.LogSeg, key messages.LogCompactionKey) error {
	if len(segments) == 0 {
		return nil
	}
...
func (p *LogCompactionProcessorV2) logLogCompactionOperation(ctx context.Context, collectorName string, inputSegments []lrdb.LogSeg, outputSegments []lrdb.LogSeg, results []parquetwriter.Result, key messages.LogCompactionKey, recordEstimate int64) error {
