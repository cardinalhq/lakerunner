...
// NewLogCompactionConsumer creates a new log compaction consumer
func NewLogCompactionConsumer(
	...
	// Create LogCompactor
	compactor := newLogCompactor(store, storageProvider, cmgr, cfg)
	...
	lcc.gatherer = newGatherer[*messages.LogCompactionMessage](topic, consumerGroup, compactor, lcc)
...
func (c *LogCompactionConsumer) Run(ctx context.Context) error {
	...
	err := c.consumer.Consume(ctx, func(ctx context.Context, kafkaMessages []fly.ConsumedMessage) error {
		...
		for _, kafkaMsg := range kafkaMessages {
			var notification messages.LogCompactionMessage
			if err := notification.Unmarshal(kafkaMsg.Value); err != nil {
				ll.Error("Failed to unmarshal log compaction message",
					slog.Any("error", err),
...
// OffsetCallbacks implementation - LogCompactionConsumer implements OffsetCallbacks interface

// GetLastProcessedOffset returns the last processed offset for this key, or -1 if never seen
func (c *LogCompactionConsumer) GetLastProcessedOffset(ctx context.Context, metadata *messageMetadata, groupingKey messages.LogCompactionKey) (int64, error) {
...
func (c *LogCompactionConsumer) MarkOffsetsProcessed(ctx context.Context, key messages.LogCompactionKey, offsets map[int32]int64) error {
	ll := logctx.FromContext(ctx)
...
func (c *LogCompactionConsumer) periodicFlush(ctx context.Context) {
	ll := logctx.FromContext(ctx)
	...
	for {
		...
		case <-c.flushTicker.C:
			...
			if _, err := c.gatherer.flushStaleGroups(ctx, 5*time.Minute, 100); err != nil {
