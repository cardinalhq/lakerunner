...
// WriterOptions returns optimized parquet writer options for the given schema and temp directory.
...
var dictionaryFieldOverride = map[string]bool{
	"_cardinalhq_message": false,
	"_cardinalhq_tid":     false,
}

// wantDictionary returns true if the field should use dictionary encoding.
// The default is true, but can be overridden in the dictionaryFieldOverride map.
...
// This is useful for dynamically creating schemas from data.
...
// NodesFromMap dynamically creates parquet nodes from a map of column names to sample values.
// This is useful for schema inference from data.
func NodesFromMap(nodes map[string]parquet.Node, sampleData map[string]any) error {
	for k, v := range sampleData {
		...
		if existingNode, ok := nodes[k]; ok {
			if !parquet.EqualNodes(existingNode, node) {
				return fmt.Errorf("type mismatch for key %s: existing %T, new %T", k, existingNode, node)
			}
			continue
		}
		nodes[k] = node
...
// ParquetSchemaFromNodemap creates a parquet schema from a map of field names to nodes.
