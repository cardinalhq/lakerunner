...
upd AS (
  ...
  RETURNING q.id, q.queue_ts, q.priority, q.organization_id, q.dateint, q.frequency_ms, q.instance_num, q.slot_id, q.slot_count, q.tries, q.claimed_by, q.claimed_at, q.heartbeated_at, q.segment_id, q.record_count, q.rollup_group, q.window_close_ts, q.eligible_at
)
SELECT 
  upd.id, upd.queue_ts, upd.priority, upd.organization_id, upd.dateint, upd.frequency_ms, upd.instance_num, upd.slot_id, upd.slot_count, upd.tries, upd.claimed_by, upd.claimed_at, upd.heartbeated_at, upd.segment_id, upd.record_count, upd.rollup_group, upd.window_close_ts, upd.eligible_at,
