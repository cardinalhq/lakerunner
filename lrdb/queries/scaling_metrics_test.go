...
func TestInqueueScalingDepth(t *testing.T) {
	...
	tests := []struct {
		...
		setupData     []lrdb.PutInqueueWorkParams
		expectedCount int64
	}{
		{
			name:          "empty queue",
		...
		{
			name:          "logs queue with unclaimed items",
			telemetryType: "logs",
			setupData: []lrdb.PutInqueueWorkParams{
				{
		...
		{
			name:          "metrics queue with different telemetry types",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := db.Pool().Exec(ctx, "TRUNCATE TABLE inqueue")
			require.NoError(t, err)

			// Setup test data
			for _, item := range tt.setupData {
				err := store.PutInqueueWork(ctx, item)
				require.NoError(t, err)
			}

			// Test the query
			result, err := store.InqueueScalingDepth(ctx, tt.telemetryType)
			require.NoError(t, err)
...
func TestInqueueScalingDepthWithClaimedItems(t *testing.T) {
	ctx := context.Background()
	store := testhelpers.NewTestLRDBStore(t)
	db := store.(*lrdb.Store) // Cast to concrete type to access Pool()
	...
	// Add unclaimed items
	unclaimedItems := []lrdb.PutInqueueWorkParams{
		{
			OrganizationID: orgID,
			CollectorName:  "collector1",
			InstanceNum:    1,
	...
	for _, item := range unclaimedItems {
		err := db.PutInqueueWork(ctx, item)
		require.NoError(t, err)
	}

	// Add a claimed item
	claimedItem := lrdb.PutInqueueWorkParams{
		OrganizationID: orgID,
		CollectorName:  "collector1",
		InstanceNum:    1,
		Bucket:         "test-bucket",
	...
	err = store.PutInqueueWork(ctx, claimedItem)
	require.NoError(t, err)
	...
	count, ok := result.(int64)
	require.True(t, ok, "Expected int64 result, got %T", result)
	assert.Equal(t, int64(2), count, "Should only count unclaimed items")
}
