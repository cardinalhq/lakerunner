...
func GetCompactCmd() *cobra.Command {
	...
	compactCmd := &cobra.Command{
		Use:   "compact",
		Short: "Add log compaction work queue items",
		Long: `Add log compaction work queue items for specified organization, instance, slot, and time.
...
Times will be aligned to hour boundaries as logs are processed hourly.`,
...
func runLogCompact(orgID uuid.UUID, instance int16, startTime time.Time, endTime *time.Time, dryRun bool) error {
	...
	if endTime == nil {
		fmt.Printf("[DRY-RUN] Would queue 1 log compaction work item for org %s instance %d at %v\n",
	...
	fmt.Printf("[DRY-RUN] Would queue %d log compaction work items for org %s instance %d from %v to %v\n",
