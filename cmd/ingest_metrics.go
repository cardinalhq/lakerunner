...
import (
	...
	"github.com/cardinalhq/lakerunner/pkg/fileconv/proto"
...
func metricIngestItem(ctx context.Context, ll *slog.Logger, sp storageprofile.StorageProfileProvider, mdb lrdb.StoreFull,
	awsmanager *awsclient.Manager, inf lrdb.Inqueue, ingest_dateint int32) error {
	tmpdir, err := os.MkdirTemp("", "lakerunner-ingest-metrics")
	if err != nil {
	...
	defer func() {
		if err := os.RemoveAll(tmpdir); err != nil {
			ll.Error("Failed to remove temporary directory", slog.Any("error", err), slog.String("dir", tmpdir))
		}
	}()
	...
	if !strings.HasPrefix(inf.ObjectID, "otel-raw/") {
		...
		if fnames, err := convertMetricsFileIfSupported(ll, tmpfilename, tmpdir, inf.Bucket, inf.ObjectID); err != nil {
	...
	for _, fname := range filenames {
		...
		if err := crunchMetricFile(ctx, ll, tmpdir, fh, inf, s3client, mdb, ingest_dateint); err != nil {
...
func crunchMetricFile(ctx context.Context, ll *slog.Logger, tmpdir string, fh *filecrunch.FileHandle, inf lrdb.Inqueue, s3client *awsclient.S3Client, mdb lrdb.StoreFull, ingest_dateint int32) error {
	...
	for blocknum, block := range blocks {
		...
		err := writeMetricSketchParquet(ctx, tmpdir, blocknum, block, inf, s3client, ll, mdb, ingest_dateint)
		if err != nil {
...
func writeMetricSketchParquet(ctx context.Context, tmpdir string, blocknum int64, block *TimeBlock, inf lrdb.Inqueue, s3client *awsclient.S3Client, ll *slog.Logger, mdb lrdb.StoreFull, ingest_dateint int32) error {
	...
	pw, err := buffet.NewWriter("metrics", tmpdir, nodes, targetFileSize, 100)
	if err != nil {
...
func convertMetricsFileIfSupported(ll *slog.Logger, tmpfilename, tmpdir, bucket, objectID string) ([]string, error) {
	switch {
	case strings.HasSuffix(objectID, ".binpb"):
		return convertMetricsProtoFile(tmpfilename, tmpdir, bucket, objectID)
	default:
...
func convertMetricsProtoFile(tmpfilename, tmpdir, bucket, objectID string) ([]string, error) {
