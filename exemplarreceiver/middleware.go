...
func (r *ReceiverService) apiKeyMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if apiKey != "" {
			...
			orgID, err = r.extractOrgIDFromJWT(req)
			if err != nil {
...
func (r *ReceiverService) extractOrgIDFromJWT(req *http.Request) (*uuid.UUID, error) {
	// Get cardinal_token cookie
	cookie, err := req.Cookie("cardinal_token")
	if err != nil {
	...
	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		...
		return []byte(r.jwtSecretKey), nil
	})
